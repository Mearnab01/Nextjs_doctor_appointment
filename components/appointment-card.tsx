...
import { ActionType, AppointmentCardProps } from "@/types";
...
export function AppointmentCard({
  ...
  const {
    ...
    fn: submitCancel,
    data: cancelData,
  } = useFetch<[FormData], { success: boolean }>(cancelAppointment);

  const {
    loading: notesLoading,
    fn: submitNotes,
    data: notesData,
  } = useFetch<[FormData], { success: boolean }>(addAppointmentNotes);
  ...
  const {
    ...
    [FormData],
    { success: boolean; token: string; videoSessionId: string }
  >(generateVideoToken);
  ...
  const {
    ...
    data: completeData,
  } = useFetch<[FormData], { success: boolean }>(markAppointmentCompleted);

  const formatDateTime = (value: string): string =>
    format(new Date(value), "MMMM d, yyyy 'at' h:mm a");
  ...
  const handleJoinVideoCall = async (): Promise<void> => {
