 ...
 */
interface UpdateDoctorStatusFormData {
    doctorId: string;
    status: string;
}
 ...
 */
interface UpdateDoctorActiveStatusFormData {
    doctorId: string;
    suspend: string;
}
...
export async function updateDoctorActiveStatus(formData: FormData): Promise<{ success: boolean }> {
    ...
    try {
        ...
        throw new Error(`Failed to update doctor status: ${(error as Error).message}`);
    }
}
 ...
 */
interface ApprovePayoutFormData {
    payoutId: string;
}

export async function approvePayout(formData: FormData): Promise<{ success: boolean }> {
    const isAdmin = await verifyAdmin();
    if (!isAdmin) throw new Error("Unauthorized");
